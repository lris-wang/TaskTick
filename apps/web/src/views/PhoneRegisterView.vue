...
import { phoneRegister, sendPhoneVerifyCode } from "../api";

const router = useRouter();

/** 0 = enter phone, 1 = code+password+confirm, 2 = success */
const step = ref(0);
...
async function onSendCode() {
  ...
  if (!p) { errorText.value = "请输入手机号"; return; }
  if (!/^1[3-9]\d{9}$/.test(p)) { errorText.value = "请输入有效手机号"; return; }

  sendingCode.value = true;
  try {
    const result = await sendPhoneVerifyCode(p);
    if (!result.ok) {
...
async function onSubmit() {
  ...
  loading.value = true;
  try {
    const data = await phoneRegister(p, code, password.value, username.value.trim() || undefined);
    if ("error" in data) {
    ...
    <NCard class="register-card" :bordered="false" size="huge">
      <template #header>
        <div class="card-header">
          <NText strong style="font-size:18px">手机号注册 TaskTick</NText>
      ...
      <NForm v-else-if="step === 1" @submit.prevent="onSubmit">
        <NText depth="3" style="display:block;margin-bottom:16px;font-size:13px">
          验证码已发送至 <strong>{{ phone }}</strong>，请查收。
        </NText>

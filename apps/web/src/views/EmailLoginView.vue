...
function safeRedirectPath(raw: unknown): string {
  ...
  if (raw === "/email-login" || raw.startsWith("/email-login?")) return "/";
  return raw;
}
...
<template>
  <div class="login-page">
    <NCard class="login-card" title="邮箱登录" :bordered="false" size="huge">
      <NForm @submit.prevent="onSubmit">
        <NFormItem label="邮箱">
          <NInput v-model:value="email" placeholder="your@email.com" autocomplete="email" :disabled="loading" size="large" />
        </NFormItem>
        <NFormItem label="密码">
          <NInput
